...
export default class ClaudeCliPlugin extends Plugin {
    ...
    const walk = (f: TFolder) => {
      for (const child of f.children) {
        if (child instanceof TFile && child.extension === "md") {
          // The generated reference doc is not an automation (no schedule).
  ...
  async createExampleAutomation(): Promise<{ example: TFile; docs: TFile }> {
...
class ClaudeCliSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc("Write a hello-world automation file plus an AUTOMATION-DOCS.md reference (every option explained) into the folder above.")
      .addButton((btn) =>
        btn
          ...
          .onClick(async () => {
            try {
              ...
              new Notice(`Created ${example.path} and ${docs.path}`, 4000);
